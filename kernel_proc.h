...
/*
 * Process Thread Control Block. This structure holds information on Threads handled by the same process.
 * Its necessity appears when a Thread has been deleted but its information is needed for the continuation
 * of the rest of the Threads.
 */

typedef struct process_thread_control_block {

  TCB* tcb;         /* Thread Control block associated. */            

  Task task;        // Task that this Thread will handle.
  int argl;         /**< @brief The main thread's argument length */
  void* args;       /**< @brief The main thread's argument string */

  int exitval;      /**< @brief The exit value of the thread. Given by ThreadExit */

  int exited;       /**< Flag that indicated if Thread has been deleted. **/
  int detached;     /**< Flag that indicated if Thread has been detached. **/
  ...
  int refcount;     /**< @brief A counter that holds the number of other Threads that have joined this Thread. Essential**/

  rlnode ptcb_list_node;  /**< @brief Intrusive node for @c PTCB_list */
