...
/*
BOOT_TEST(test_CreateThread,
	 "Test that checks if CreateThread works correctly")
{
	PTCB* ptcb;

	ptcb = (PTCB*)CreateThread(NULL, 0, NULL);

	ASSERT(ptcb->tcb == cur_thread());
	ASSERT(CURPROC->thread_count == 1);
	ASSERT(is_rlist_empty(&CURPROC->PTCB_list)==0);
	return 0;
}
*/
...
int run_get_status(Task task, int argl, void* args)
{
	...
	ASSERT(pid!=NOPROC);
	//fprintf(stderr,"We reached at this point.\n");
	int exitval;
	//fprintf(stderr,"Exitval at this point is: %d\n", exitval);
...
//MY TEST-----> TEST TEST-----> DOESN'T WORK CURRENTLY.
BOOT_TEST(test_detach_exited,
	 "Test that a thread cannot detach if exited")
{
	Tid_t t = CreateThread(NULL, 0, NULL);
	PTCB* ptcb = (PTCB*)t;


	ASSERT(ThreadDetach((Tid_t)ptcb)==-1);
	return 0;
}
...
int data_producer(int argl, void* args)
{
	...
	//fprintf(stderr, "nbytes written: %d\n", nbytes);
	Close(1);
	return 0;
}
...
int data_consumer(int argl, void* args) 
{
	...
	while(rc) {
		...
		assert(rc>=0);
		count += rc;
		//fprintf(stderr, "data read: %d, count: %d\n", rc, count);
	}
	//fprintf(stderr, "Count is: %d, nbytes are: %d\n", count, nbytes);
	ASSERT(count == nbytes);
	return 0;
}
